...
const SUB_STORE_SCHEMA = {
  ...
  scope: ["Surge", "Loon", "Quantumult X"], // 增加 Quantumult X
  author: "@Key @奶茶姐 @小一 @可莉",
  updateTime: "2024-01-29", // 建议更新日期
  version: "1.2.3",
  params: {
    flag: {
      datatype: "boolean",
...
const { isLoon, isSurge, isQuanX } = $substore.env;
const XHFGF = iar.sn == undefined ? " " : decodeURI(iar.sn);
const dns = iar.dnsjx;
const keypr = "peedtest";
// 使用 let 声明 target，允许后面在 operator 函数中重新赋值
let target = isLoon ? "Loon" : isSurge ? "Surge" : isQuanX ? "Quantumult X" : undefined;
...
async function operator(e = [], targetPlatform, env) {
  ...
  // 通过传入的 targetPlatform 再次确认支持状态
  const support = isLoon || isSurge || isQuanX || targetPlatform === "Quantumult X";
  if (targetPlatform === "Quantumult X") target = "Quantumult X";
  if (!xy) {
    if (!support) {
      $.notify("不支持当前平台", `识别到的平台: ${targetPlatform}`, "请检查 Sub-Store 运行环境");
	  $.error(`不支持当前平台: ${targetPlatform}`);
      return e;
    }
  }
        ...
        batch.map(async (pk) => {
            try {
              ...
              if (!xy || yisp || yw || flag) {
                if (!support) {
                  $.notify("当前平台不支持落地查询")
                  $.error(`当前平台不支持落地查询, 开启 yisp || yw || flag 参数后 xy 参数无效`);
                    return e;
                  }
