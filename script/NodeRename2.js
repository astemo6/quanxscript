...
const XHFGF = iar.sn == undefined ? " " : decodeURI(iar.sn),
      { isLoon, isSurge, isQuanX } = $substore.env, // 增加 isQuanX
      dns = iar.dnsjx,
      target = isLoon ? "Loon" : isSurge ? "Surge" : isQuanX ? "Quantumult X" : undefined, // 增加目标判断
      keypr= "peedtest";
...
async function operator(e = [], targetPlatform, env) {
  ...
  const support = isLoon || isSurge || isQuanX; // 增加 isQuanX
  if (!xy) {
    if (!support) {
      $.notify("不支持当前平台")
      $.error(`不支持当前平台`);
