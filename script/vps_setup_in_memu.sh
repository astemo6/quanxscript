#!/bin/bash
# Scripts to start in a new VPS_memu to choose
...
# Function to configure ports, apply certificates, and configure auto-renewal
...
show_menu() {
    ...
    echo "5) Install x-ui"
    echo "6) Configure ports and certificates"
    echo "7) Open additional ports"
    echo "8) Enable BBR"
    echo "9) Exit"
    echo "====================================================="
    ...
    case $option in
        ...
        4) install_docker ;;
        5) install_xui ;;
        6) configure_ports_and_certificates ;;
        7) open_additional_ports ;;
        8) enable_bbr ;;
        9) exit 0 ;;
        *) echo "Invalid option. Please choose between 1 and 9."; show_menu ;;
    esac
}

# Main function to start menu
main() {
    while true; do
        show_menu
    done
